...
struct Coefficient {
  Value positiveFactor; // read: positive part of this coefficient's constant factor
  Value negativeFactor; // read: negative part of this coefficient's constant factor
                        // hence: the value of this coefficient's constant factor
                        //        is equal to ('positiveFactor' - 'negativeFactor') 
  StringVector parameters;
...
private:
  ExprRef preFactorExpr() const {
    ...
    if (positiveFactor == 0) {
      ...
      expr = std::make_shared<BinOp>(MINUS,
				     std::make_shared<Constant>(positiveFactor),
...
struct Monomial {
  ...
  void normalize() {
    ...
    for (auto cc: combined) {
      coefficients.push_back({cc.second.positive, cc.second.negative, cc.first});
...
struct MonomialCollection : public Analysis, ExprVisitor {
  void init() final { monomials.clear(); }
...
private:
  ...
  void visitBinOp(const BinOp *b) final {
    ...
    Value factor = 1;
    for (const auto c: AC.getConstants()) { factor *= c; }
